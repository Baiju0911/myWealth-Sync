...
class StatementStagingCommitView(APIView):
    ...
    🔒 CORE TRANSACTION COMMIT ENGINE (PRODUCTION RECTIFIED):
    Maps, validates, and atomically commits both registry headers and transactional children blocks.
    Defensively intercepts line-item splits to prevent metadata aggregation drift on re-uploads.
    """
    ...
    def post(self, request, *args, **kwargs):
        ...
        # 🎯 EXTRACT ADMIN NOTES TELEMETRY DIRECTLY FROM REACT PREVIEW WRAPPERS
        admin_notes_count_fallback = int(
            meta_summary.get("emptyMemoLineCount")
            or meta_summary.get("admin_notes_count")
            or 0
        )
        ...
        # 3. Construct lookup dictionary for hashing duplicates
        db_records = StatementStagingLine.objects.filter(account_id=account.id).values(
            "row_identifier",
            "narration",
            "cheque_ref_number",
        ...
        try:
            ...
            with transaction.atomic():
                # ─── 🏛️ STEP 4: PRE-SCAN TX_POOL TO CALCULATE LIVE COMMITTED VALUES ───
                actual_insert_count = 0
                actual_update_count = 0
                actual_debit_lines = 0
                actual_credit_lines = 0
                actual_admin_notes = 0

                for tx in tx_pool:
                    action_intent = tx.get("pipeline_action", "INSERT")

                    val_deb = tx.get("debit")
                    val_cred = tx.get("credit")

                    is_debit = val_deb and str(val_deb).strip() not in (
                        "-",
                        "",
                        "0.00",
                        "0",
                    )
                    is_credit = val_cred and str(val_cred).strip() not in (
                        "-",
                        "",
                        "0.00",
                        "0",
                    ...
                    if action_intent == "UPDATE_ENRICHMENT":
                        ...
                        if is_debit:
                            actual_debit_lines += 1
                        elif is_credit:
                            actual_credit_lines += 1
                        else:
                            # 🎯 IDENTIFIES AN EXPLICIT NON-FINANCIAL SYSTEM NOISE/ADMIN NOTE RECORD IN THE POOL
                            actual_admin_notes += 1

                # Use live pool calculation if notes exist, otherwise fallback to frontend telemetry payload values
                final_admin_notes_logged = (
                    actual_admin_notes
                    if actual_admin_notes > 0
                    else admin_notes_count_fallback
                )

                # ─── TABLE 1 WRITER: Hardened Clean Registry Header Write ───
                registry_entry = StatementIngestRegistry.objects.create(
                    account=account,
                    file_name=file_name,
                    file_type=clean_file_type,
                    ...
                    total_debit_amount=tot_dr,
                    total_credit_amount=tot_cr,
                    # 🎯 SAVES TELEMETRY BALANCED COLUMNS DIRECTLY INTO CORE DB SCHEMA FIELD KEYS
                    total_row_count=actual_insert_count,
                    debit_line_count=actual_debit_lines,
                    credit_line_count=actual_credit_lines,
                    admin_notes_count=final_admin_notes_logged,
                    skipped_duplicate_count=(
                        duplicate_skip_count + actual_update_count
                    ),
                    source_channel="WEB_DASHBOARD",
                )
            ...
            return Response(
                {
                    "status": "SUCCESS",
                    "registry_id": str(registry_entry.id),
                    "message": f"Sync run complete. Saved {len(create_pool)} new rows (including {final_admin_notes_logged} admin note entries), enriched & updated {update_records_count} transactions, skipped {duplicate_skip_count} unchanged duplicates.",
                },
                status=status.HTTP_200_OK,
            )
...
class StatementBulkIngestPipelineView(APIView):
    ...
    Loads historical deduplication hashes up front, processes document layouts,
    normalizes string variations, and triggers inline narration enrichment checks
    before emitting the payload grid to the front-end dashboard preview with matching strategy signatures.
    """
    ...
    def post(self, request, *args, **kwargs):
        ...
        try:
            ...
            # Fallback initialization target signature
            active_strategy = getattr(
                template_obj, "parser_strategy_code", "UNIVERSAL_CSV_FLOW"
            ).upper()
            ...
            if (
                ...
                return Response(
                    {
                        "status": "SUCCESS",
                        "strategy_processed": "UNIVERSAL_CSV_FLOW",
                        "engine_strategy_executed": "UNIVERSAL_CSV_FLOW",
                        "export_filename": export_filename,
                        "raw_csv_stream": processed_bundle,
                        "preview_dataset": [],
                        "opening_balance": op_bal,
                ...
                # 🎯 THE CORE BACKEND RESOLUTION INTERCEPT:
                # Extract the dynamic strategy assigned directly by process_bank_statement loops
                if intermediate_txns and isinstance(intermediate_txns, list):
                    first_tx_element = intermediate_txns[0]
                    if (
                        isinstance(first_tx_element, dict)
                        and "strategy_used" in first_tx_element
                    ):
                        active_strategy = str(
                            first_tx_element.get("strategy_used")
                        ).upper()
            ...
            return Response(
                {
                    "status": "SUCCESS",
                    # 🎯 EMITS THE REAL STRATEGY STRAIGHT TO THE FRONT-END
                    "strategy_processed": active_strategy,
                    "engine_strategy_executed": active_strategy,
                    "confidence_score": ocr_confidence_score,
                    "system_noise_records_cleared": (
