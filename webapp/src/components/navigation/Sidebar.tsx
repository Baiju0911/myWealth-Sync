...
export default function Sidebar({ currentView, setCurrentView }: SidebarProps) {
  ...
  return (
    ...
    >
      ...
      <div 
        className={`border-b border-zinc-800 flex items-center gap-3 shrink-0 ${
      ...
      >
        ...
        {!isCollapsed && (
          <div className="animate-fade-in text-left min-w-0">
            <h1 className="font-bold text-sm tracking-tight text-white whitespace-nowrap m-0 leading-none" style={{ fontSize: '14px', margin: 0 }}>
              myWealth-Sync
            </h1>
            <p className="text-[10px] text-zinc-500 font-mono m-0 mt-0.5" style={{ margin: 0 }}>
              v1.0 (MySQL)
            </p>
          </div>
        )}
      ...
      <nav 
        className="flex-1 p-2.5 space-y-1.5 overflow-y-auto w-full flex flex-col custom-scrollbar" 
          ...
          return (
            <button
              ...
              className={`w-full flex items-center rounded-lg text-xs font-medium transition-all duration-200 cursor-pointer ${
                isCollapsed ? 'justify-center p-3 gap-0' : 'px-3.5 py-2.5 gap-2.5 justify-start text-left'
              } ${
                isActive
            ...
            >
              ...
              {!isCollapsed && (
                <span className="animate-fade-in text-[12px] font-sans leading-tight">
                  {item.label}
                </span>
              )}
            </button>
      ...
      {/* 3. User Context Profile Footer */}
      <div 
        className={`p-3 border-t border-zinc-800 bg-zinc-950/60 flex items-center gap-3 shrink-0 mt-auto ${
      ...
      >
        ...
        {!isCollapsed && (
          <div className="truncate animate-fade-in text-left min-w-0">
            <p className="text-xs font-semibold text-zinc-300 truncate m-0" style={{ margin: 0 }}>
              Alpha Tester
            </p>
            <p className="text-[9px] text-zinc-500 font-mono truncate m-0" style={{ margin: 0 }}>
              testowner@wealth.com
            </p>
          </div>
        )}
      </div>
